...
async def ai_ask_main(uid: str, sid: str, persona: str | None, text: str, chat_id: str | None) -> str:
    ...
    await add_event("system", "明显在讨论 CS、比赛、天梯、官匹、rating、WE、ADR、胜率、队友、双排、排名或某个玩家表现时，要用数据说话。聊天记录检索只能说明群里谁说了什么、大家在讨论什么，不能替代 CS 数据证据；不要分析完聊天记录就结束。若问题是在评判玩家表现或验证群聊里的游戏结论，应继续调用 fetch_user_summary、fetch_duo_summary、fetch_teammate_ranking 或 fetch_group_rankings 等 CS 数据工具，再结合聊天记录和数据回答。只有当用户明确只问“群里谁说过/怎么说的”时，才可以只用聊天记录作答。")
