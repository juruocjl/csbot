...
async def ai_ask_main(uid: str, sid: str, persona: str | None, text: str, chat_id: str | None) -> str:
    ...
    await add_event("system", "明显在讨论 CS、比赛、天梯、官匹、rating、WE、ADR、胜率、队友、双排、排名或某个玩家表现时，应同时参考群聊记录和 CS 数据，不要看完一边就不管另一边。聊天记录能说明群里谁说了什么、争论点是什么、问题从哪里来；CS 数据能说明实际表现、趋势和排名。若问题是在评判玩家表现、验证群聊里的游戏结论、解释某个比赛/数据相关争论，应先用 search_chat_spans 理解讨论上下文，再调用 fetch_user_summary、fetch_duo_summary、fetch_teammate_ranking 或 fetch_group_rankings 等 CS 数据工具，最终回答同时交代“群聊依据”和“数据依据”。只有当用户明确只问“群里谁说过/怎么说的”时，才可以只用聊天记录；只有当用户明确只要查数据且不关心群聊上下文时，才可以只用 CS 数据。")
