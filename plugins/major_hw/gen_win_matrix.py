...
def print_win_matrix(win_matrix: dict[str, dict[str, float]], teams: list[Team]) -> None:
    column_width = 10
    header = "Team".center(column_width)
    for team in teams:
        team_name = team.name
...
def load_teams(file_path: str | Path, include_rating_teams: bool = False) -> list[Team]:
    with open(file_path) as file:
        data = json.load(file)

    teams = []
    team_sections = [data["teams"]]
    if include_rating_teams:
        team_sections.append(data.get("rating_teams", {}))

    for team_section in team_sections:
        for team_name, team_data in team_section.items():
            rating = tuple(
...
def resolve_team_name(
    ...
    match = process.extractOne(raw_name, alias2full.keys())
    if match is None:
        return None
    matched_name, score = match
    if score < min_score:
        return None
    return alias2full[matched_name]
...
def apply_finished_matches_to_ratings(
    ...
    for match_index, (winner_raw, loser_raw, score, _) in enumerate(matches):
        winner_name = resolve_team_name(winner_raw, alias2full)
        loser_name = resolve_team_name(loser_raw, alias2full)
...
def gen_win_matrix(
    ...
    finish_match: list[tuple[str, str, str, str]],
    newest_first: bool = False,
    rating_match: list[tuple[str, str, str, str]] | None = None,
):
    teams = load_teams(file_path)
    rating_teams = load_teams(file_path, include_rating_teams=True)
    ...
    rating_teams = apply_finished_matches_to_ratings(
        rating_teams,
        system_names,
        rating_match if rating_match is not None else finish_match,
        alias2full,
        newest_first=newest_first,
    )
    rating_by_name = {team.name: team.rating for team in rating_teams}
    teams = [
        replace(team, rating=rating_by_name.get(team.name, team.rating))
        for team in teams
    ]
    ...
    lock_alias2full = {}
    for team in teams:
        lock_alias2full[team.name] = team.name
        for alias in team.alias:
            lock_alias2full[alias] = team.name

    for teama, teamb, _, _ in finish_match:
        teama = resolve_team_name(teama, lock_alias2full)
        teamb = resolve_team_name(teamb, lock_alias2full)
    ...
    logger.info("win matrix saved to win_matrix.csv")
